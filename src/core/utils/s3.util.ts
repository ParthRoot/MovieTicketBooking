...
export class S3Util {
    ...
    constructor() {
        ...
        this._excelbucket = getEnv('AWS_EXCEL_BUCKET_NAME');
    ...
    async getPreSignedUploadRequestData(contentType: string, $key: string) {
        ...
        const presignedUrl = await getSignedUrl(this._client, command, { expiresIn: parseInt(this.ttl) });
        return presignedUrl;
    }

    preSignUrl(fileKey: string, bucket: string, fileName?: string): Promise<string> {
    ...
    async deleteObjectByUrl(s3Url: string) {
        try {
            ...
            if (bucketName) {
                // Delete object
                await this._clients3.deleteObject({ Bucket: bucketName, Key: objectKey }).promise();
    ...
    public async getFileInfo($key: string): Promise<{ status: boolean; fileSize: number }> {
        try {
     ...
     */
    async readS3File($key: string): Promise<any> {
        return new Promise((resolve, reject) => {
            this._clients3.getObject(
                {
                    Bucket: this._excelbucket,
                    Key: $key.replace(`s3://${this._bucket}/`, ''),
                },
                (err, data) => {
                    if (err) {
    ...
    public async uploadFile(file: any, fileName: string, folderPath: string): Promise<string> {
        try {
